"""
AI-Powered Label Front Generator using SAP AI Core
Generates food-type-specific HTML/CSS for front labels
"""
...
class AILabelGenerator:
    ...
    def get_food_type_style(self, food_type):
        ...
        styles = {
            ...
            'health_drink': {
                'colors': 'clinical blues, greens, white',
                'vibe': 'trustworthy, scientific, beneficial',
                'imagery': 'health badges, nutrition symbols'
            },
            'packaged_processed_food': {
                'colors': 'warm oranges, reds, yellows',
                'vibe': 'appetizing, traditional, homely',
                'imagery': 'food items, traditional elements'
            }
        }
...
- Use inline styles within HTML
- Include complete CSS in <style> tag
- Veg symbol: {"Green" if veg_status == "veg" else "Brown"} dot in square border
- MRP format: "MRP (Incl. of all taxes)" label above price
- Net Quantity format: Label "Net Quantity" above value
...
Generate now:"""
        ...
        try:
            # Call SAP AI Core (deployment_id determines the model)
...
if __name__ == "__main__":
    ...
    print("Generating AI-powered front label...")
    html = generator.generate_front_label_html(sample_data)
    
    # Save to file
    with open('test_ai_label.html', 'w') as f:
        f.write(f"""<!DOCTYPE html>
<html>
<head>
    <meta charset="UTF-8">
    <title>AI Generated Label</title>
</head>
<body style="display: flex; justify-content: center; align-items: center; min-height: 100vh; background: #f0f0f0;">
    ...
    print("✓ AI label generated: test_ai_label.html")